...
def setup_probing():
    ...
    p2g.codenl(haas.PROBE_ON, comment_txt=p2g.stat.CType.SRC_COMMENT)
    p2g.codenl(haas.NO_LOOKAHEAD, comment_txt=p2g.stat.CType.NO_COMMENT)


def pause(txt: str):
    p2g.comment()
    p2g.message(haas.MESSAGE[0], txt)
    p2g.comment()


def alarm(txt: str):
    p2g.message(haas.ALARM[0], txt)


def ots_calibrate():
    p2g.codenl("G65 P9023 A20. K5. S0.5 D-2.", comment_txt=p2g.stat.CType.SRC_COMMENT)


def spindle_probe_find_height():
    p2g.codenl("G65 P9023 A21. T1.", comment_txt=p2g.stat.CType.SRC_COMMENT)


def spindle_probe_find_radius(diameter):
    p2g.codenl(f"G65 P9023 A10.0 D{diameter}", comment_txt=p2g.stat.CType.SRC_COMMENT)


def load_tool(tool):
    p2g.codenl(f"T{tool:02} M06", comment_txt=p2g.stat.CType.SRC_COMMENT)
...
class OTS:
    def __enter__(self):
        ...
        p2g.codenl(haas.OTS_ON, comment_txt="OTS On.")

    def __exit__(self, *_):
        p2g.codenl(haas.OTS_OFF, comment_txt="OTS Off.")
