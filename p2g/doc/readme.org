...
Names created in the ~p2g.Symbol~class are also used to make a
symbol table. 
...
#+BEGIN_SRC python
    ...
    # define st as an instance of p2g.Symbols
    st = p2g.Symbols()
    x1 = -7

    # fill with data.
    st.MACHINE_ABS_ABOVE_OTS = p2g.Const(x=x1, y=8, z=9)
    st.MACHINE_ABS_ABOVE_SEARCH_ROTARY_LHS_5X8 = p2g.Const(100, 101, 102)
    st.MACHINE_ABS_ABOVE_VICE = p2g.Const(x=17, y=18, z=19)
    st.RAW_ANALOG = p2g.Fixed[10](addr=1080)
    st.fish = 10
    st.not_used = 12

    def stest():
        p2g.comment("Only used symbols are in output table.")
        p2g.Var(st.MACHINE_ABS_ABOVE_OTS)
        p2g.Var(st.MACHINE_ABS_ABOVE_VICE * st.fish)
        v1 = p2g.Var()
        v1 += st.RAW_ANALOG[7]
#+END_SRC  
...
#+RESULTS: stest
#+begin_example
( MACHINE_ABS_ABOVE_OTS  : -7.,8.,9.   )
( MACHINE_ABS_ABOVE_VICE : 17.,18.,19. )
( RAW_ANALOG             : #1080[10]   )
( fish                   : 10          )
  O0001                           ( -                             )

( Only used symbols are in output table. )
  #100= -7.                       ( Var [st.MACHINE_ABS_ABOVE_OTS])
  #101= 8.
  #102= 9.
  #103= 170.                      ( Var [st.MACHINE_ABS_ABOVE_VICE * st.fish])
  #104= 180.
  #105= 190.
  #106= #106 + #1087              ( v1 += st.RAW_ANALOG[7]        )
  M30
#+end_example
